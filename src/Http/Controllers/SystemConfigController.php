...
class SystemConfigController extends AdminController
{
    ...
    protected function form()
    {
        return Form::make(new SystemConfigModel(), function (Form $form) {
            ...
            if ($form->isEditing()) {
                if (in_array($form->model()->type, [6,7,10,11,12,13,14])){
                 ...
                 ->when(0, function (Form $form) {
                     ...
                     $form->select('type', __('类型'))->options([
                         ...
                         12 => '复选框',
                          ...
                          ->when(14, function (Form $form) {
                              $form->divider();

                              $form->table('extra14','多选下拉框选项', function (Form\NestedForm $table) {
                                  $table->text('key', '键值(key)');
                                  $table->text('label', '标签(value)');
                              });
                          })
            ...
            $form->saving(function (Form $form) {
                ...
                if ($form->model()->config_type || $form->input('config_type')){
                    ...
                    if (in_array($form->type, [6,7,10,11,12,13,14])){
                    ...
                    if ($form->type == 10 || $form->type == 11 || $form->type == 12 || $form->type == 14) {
                        if (!$form->extra) {
    ...
    public function update($id)
    {
        try {
            ...
            if (in_array($model->type, [6,7,10,11,12,13,14])){
            ...
            if ($model->type == 10 || $model->type == 11 || $model->type == 12 || $model->type == 14) {
                if (!$request->extra) {
